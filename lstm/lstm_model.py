...
if future_db.exists('BTCUSD'):
    ...
    future_db.json().set('BTCUSD', Path.root_path(), dict())


def predict_and_write_to_redis(ticker='BTCUSD', period='MINUTE'):
    def get_and_parse_data(ticker='BTCUSD', period='MINUTE'):
    ...
    data = get_and_parse_data(ticker=ticker, period=period)
    ...
    predictions = predict(np.array(data_for_prediction), f'BTCUSD/BTCUSD-{period}.h5', n_days_for_prediction)
    ...
    future_db.json().set(ticker, Path(f'.{period}'), j)



def start_prediction():
    while True:
        for p in ['MINUTE', 'MINUTE_5', 'MINUTE_15', 'MINUTE_30', 'HOUR', 'HOUR_4', 'DAY', 'WEEK']:
            predict_and_write_to_redis(ticker='BTCUSD', period=p)
        sleep(65)

sleep(5)

start = Thread(target=start_prediction)
start.start()
