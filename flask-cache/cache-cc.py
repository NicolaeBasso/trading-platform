...
def convert_epoch(resolution=None):
    ...
    WEEK = datetime.datetime.fromtimestamp(epoch - 604800000).strftime('%Y-%m-%dT%H:%M:%S')
...
def historical_price(epic='BTCUSD', resolution='WEEK', max='1000', fro='2020-02-24T00:00:00', to='2022-02-24T01:00:00'):
    endpoint = f'https://demo-api-capital.backend-capital.com/api/v1/prices/{epic}?resolution={resolution}&max={max}&from={fro}&to={to}'
...
def update_cache():
    ...
    for res in ['MINUTE', 'MINUTE_5', 'MINUTE_15', 'MINUTE_30', 'HOUR', 'HOUR_4', 'DAY', 'WEEK']:
        to, fro = convert_epoch(res)
        data = historical_price(resolution=res, to=to, fro=fro)
